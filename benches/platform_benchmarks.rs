...
fn bench_memory_constrained(c: &mut Criterion) {
    ...
    group.bench_function("1kb_line_chart", |b| {
        let mut data = StaticDataSeries::<Point2D, 32>::new(); // ~256 bytes
        for i in 0..32 {
            data.push(Point2D::new(i as f32, (i as f32).sin() * 10.0)).ok();
        }
    ...
    group.bench_function("4kb_multi_series", |b| {
        let mut series1 = StaticDataSeries::<Point2D, 64>::new();
        let mut series2 = StaticDataSeries::<Point2D, 64>::new();
        
        for i in 0..64 {
            series1.push(Point2D::new(i as f32, (i as f32 * 0.1).sin() * 20.0)).ok();
            series2.push(Point2D::new(i as f32, (i as f32 * 0.1).cos() * 20.0)).ok();
        }
        ...
        b.iter(|| {
            ...
            let mut display = MockDisplay::<Rgb565>::new();
            let viewport = Rectangle::new(Point::zero(), Size::new(160, 120));
            
            chart.draw(&series1, chart.config(), viewport, &mut display).ok();
            chart.draw(&series2, chart.config(), viewport, &mut display).ok();
...
fn bench_streaming_scenarios(c: &mut Criterion) {
    ...
    group.bench_function("ecg_streaming_update", |b| {
        ...
        b.iter(|| {
            // Simulate adding new sample and removing old
            if buffer.len() >= 256 {
                buffer.remove(0);
            }
            let new_sample = generate_ecg_sample(sample_index);
            buffer.push(Point2D::new(sample_index as f32, new_sample)).ok();
            sample_index += 1;
            
            let mut display = MockDisplay::<Rgb565>::new();
            let viewport = Rectangle::new(Point::zero(), Size::new(320, 240));
            chart.draw(&buffer, chart.config(), viewport, &mut display).ok();
            black_box(display);
        });
    });
...
fn bench_dashboard_rendering(c: &mut Criterion) {
    ...
    group.bench_function("4_chart_dashboard", |b| {
        // Prepare data for 4 different charts
        let line_data = generate_sine_data(50);
        let bar_data = generate_random_data(10, 0.0, 100.0);
        let gauge_value = 75.0;
        ...
        let gauge_chart = GaugeChart::builder()
            .radius(50)
            .build()
            .unwrap();
        ...
        b.iter(|| {
            ...
            // Render 4 charts in quadrants
            let viewports = [
                Rectangle::new(Point::new(0, 0), Size::new(160, 120)),
                Rectangle::new(Point::new(160, 0), Size::new(160, 120)),
                Rectangle::new(Point::new(0, 120), Size::new(160, 120)),
                Rectangle::new(Point::new(160, 120), Size::new(160, 120)),
            ];
            
            line_chart.draw(&line_data, line_chart.config(), viewports[0], &mut display).ok();
            bar_chart.draw(&bar_data, bar_chart.config(), viewports[1], &mut display).ok();
            gauge_chart.draw_value(gauge_value, viewports[2], &mut display).ok();
            pie_chart.draw(&pie_data, pie_chart.config(), viewports[3], &mut display).ok();
...
fn generate_random_data(size: usize, min: f32, max: f32) -> StaticDataSeries<Point2D, 64> {
    let mut series = StaticDataSeries::new();
...
fn generate_pie_data(slices: usize) -> StaticDataSeries<Point2D, 16> {
    let mut series = StaticDataSeries::new();
    let total = 100.0;
    let slice_value = total / slices as f32;
    
    for i in 0..slices.min(16) {
