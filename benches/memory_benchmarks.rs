...
fn bench_complete_chart_memory(c: &mut Criterion) {
    ...
    group.bench_function("standard_4kb_setup", |b| {
        b.iter(|| {
            ...
            for i in 0..128 {
                data.push(Point2D::new(i as f32, (i as f32 * 0.1).sin() * 50.0)).ok();
            }
            
            let chart = LineChart::builder()
                .line_color(embedded_graphics::pixelcolor::Rgb565::BLUE)
                .line_width(2)
                .with_markers(MarkerStyle::Circle(3))
                .build()
                .unwrap();
    ...
    group.bench_function("advanced_16kb_setup", |b| {
        b.iter(|| {
            ...
            for i in 0..256 {
                data1.push(Point2D::new(i as f32, (i as f32 * 0.1).sin() * 50.0)).ok();
                data2.push(Point2D::new(i as f32, (i as f32 * 0.1).cos() * 50.0)).ok();
                data3.push(Point2D::new(i as f32, (i as f32 * 0.05).sin() * 30.0)).ok();
            }
...
fn bench_allocation_patterns(c: &mut Criterion) {
    ...
    group.bench_function("streaming_update_pattern", |b| {
        ...
        b.iter(|| {
            ...
            buffer.push(Point2D::new(index as f32, (index as f32).sin())).ok();
            index += 1;
            black_box(&buffer);
        });
    });
    ...
    group.bench_function("aggregation_pattern", |b| {
        ...
        for i in 0..1024 {
            source.push(Point2D::new(i as f32, (i as f32 * 0.01).sin() * 100.0)).ok();
        }
...
fn bench_feature_memory_overhead(c: &mut Criterion) {
    ...
    group.bench_function("line_chart_with_markers", |b| {
        b.iter(|| {
            let chart = LineChart::builder()
                .line_color(embedded_graphics::pixelcolor::Rgb565::BLUE)
                .with_markers(MarkerStyle::Circle(4))
                .build()
                .unwrap();
            black_box(mem::size_of_val(&chart));
        });
    ...
    group.bench_function("line_chart_with_area", |b| {
        b.iter(|| {
            let chart = LineChart::builder()
                .line_color(embedded_graphics::pixelcolor::Rgb565::BLUE)
                .with_area_fill(embedded_graphics::pixelcolor::Rgb565::new(0, 0, 255))
                .build()
                .unwrap();
            black_box(mem::size_of_val(&chart));
        });
    ...
    group.bench_function("line_chart_all_features", |b| {
        b.iter(|| {
            let chart = LineChart::builder()
                .line_color(embedded_graphics::pixelcolor::Rgb565::BLUE)
                .line_width(3)
                .with_markers(MarkerStyle::Square(5))
                .with_area_fill(embedded_graphics::pixelcolor::Rgb565::new(0, 0, 255))
...
fn generate_memory_report() {
    ...
    // Configuration sizes
    println!("\nConfiguration Sizes:");
    println!("  ChartConfig: {} bytes", mem::size_of::<ChartConfig>());
