...
fn test_chart_error_display() {
    let errors = vec![
        (ChartError::InsufficientData, "Insufficient data to render the chart"),
        (ChartError::InvalidRange, "Invalid range specified for axis or data"),
        (ChartError::InvalidData, "Invalid data provided"),
        (ChartError::MemoryFull, "Memory allocation failed or buffer is full"),
        (ChartError::RenderingError, "Error occurred during rendering"),
        (ChartError::InvalidConfiguration, "Invalid configuration provided"),
        (ChartError::ConfigurationError, "Configuration error occurred"),
    ];
...
fn test_data_error_constants() {
    assert_eq!(DataError::SERIES_NOT_FOUND, DataError::SeriesNotFound { context: None });
    assert_eq!(DataError::INDEX_OUT_OF_BOUNDS, DataError::IndexOutOfBounds { context: None });
    assert_eq!(DataError::INVALID_DATA_POINT, DataError::InvalidDataPoint { context: None });
    assert_eq!(DataError::SCALING_ERROR, DataError::ScalingError { context: None });
    assert_eq!(DataError::BUFFER_FULL, DataError::BufferFull { context: None });
    assert_eq!(DataError::INSUFFICIENT_DATA, DataError::InsufficientData { context: None });
}
...
fn test_data_error_simple() {
    let kinds = vec![
        (DataErrorKind::SeriesNotFound, DataError::SeriesNotFound { context: None }),
        (DataErrorKind::IndexOutOfBounds, DataError::IndexOutOfBounds { context: None }),
        (DataErrorKind::InvalidDataPoint, DataError::InvalidDataPoint { context: None }),
        (DataErrorKind::ScalingError, DataError::ScalingError { context: None }),
        (DataErrorKind::BufferFull, DataError::BufferFull { context: None }),
        (DataErrorKind::InsufficientData, DataError::InsufficientData { context: None }),
    ];
...
fn test_data_error_display() {
    ...
    let err = DataError::BufferFull {
        context: Some(ErrorContext::with_numeric("push_data", "Remove old data", 512)),
    };
    ...
    let err = DataError::IndexOutOfBounds {
        context: Some(ErrorContext::with_numeric("array_access", "Check bounds", 10)),
    };
    let display = format!("{}", err);
...
fn test_animation_error_display() {
    let errors = vec![
        (AnimationError::InvalidDuration, "Invalid duration specified"),
        (AnimationError::AnimationNotFound, "Animation with specified ID was not found"),
        (AnimationError::SchedulerFull, "Animation scheduler is full"),
        (AnimationError::InterpolationError, "Error occurred during interpolation"),
...
fn test_layout_error_display() {
    let errors = vec![
        (LayoutError::InsufficientSpace, "Insufficient space for layout"),
        (LayoutError::InvalidConfiguration, "Invalid layout configuration"),
        (LayoutError::PositioningFailed, "Component positioning failed"),
    ];
...
fn test_render_error_display() {
    let errors = vec![
        ...
        (RenderError::ColorConversionFailed, "Color conversion failed"),
    ];
...
fn test_error_equality() {
    ...
    assert_eq!(LayoutError::InsufficientSpace, LayoutError::InsufficientSpace);
    assert_ne!(LayoutError::InsufficientSpace, LayoutError::InvalidConfiguration);
...
fn test_error_copy_clone() {
    ...
    let _chart_clone = chart_err.clone(); // Clone

    let data_err = DataError::BUFFER_FULL;
    let _data_copy = data_err; // Copy
    let _data_clone = data_err.clone(); // Clone

    let layout_err = LayoutError::InsufficientSpace;
    let _layout_copy = layout_err; // Copy
    let _layout_clone = layout_err.clone(); // Clone

    let render_err = RenderError::DrawingFailed;
    let _render_copy = render_err; // Copy
    let _render_clone = render_err.clone(); // Clone
}
...
fn test_result_type_aliases() {
    ...
    let ok_result: ChartResult<i32> = Ok(42);
    ...
    let layout_result: LayoutResult<f32> = Ok(3.14);
    assert_eq!(layout_result.unwrap(), 3.14);
...
fn test_animation_result_type_alias() {
    ...
    let ok_result: AnimationResult<u32> = Ok(123);
