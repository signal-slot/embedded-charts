...
use embedded_charts::math::interpolation::{InterpolationType, InterpolationConfig};
...
use embedded_charts::axes::{LinearAxis, AxisOrientation, AxisPosition};
...
fn test_curve_chart_config_setters() {
    ...
    let config = ChartConfig {
        ...
        margins: Margins::new(10, 20, 30, 40),
        show_grid: false,
        grid_color: None,
    };
    ...
    assert_eq!(chart.config().title.as_ref().map(|s| s.as_str()), Some("Test Chart"));
...
fn test_curve_chart_builder_all_options() {
    ...
    assert!(chart.style().markers.is_some());
    assert_eq!(chart.config().title.as_ref().map(|s| s.as_str()), Some("Curve Test"));
    assert_eq!(chart.config().background_color, Some(Rgb565::BLACK));
}
...
fn test_curve_chart_different_interpolation_types() {
    ...
    for interp_type in interpolation_types {
        ...
        let result = chart.draw(&data, &config, viewport, &mut display);
        assert!(result.is_ok(), "Failed for interpolation type: {:?}", interp_type);
    }
}
